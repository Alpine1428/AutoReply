...
public class ResponseEngine {
    ...
    private void buildRules() {
        ...
        // ==================== 500: ПРЯМОЙ ОТКАЗ (ключевое слово) ====================

        rules.add(new Rule("ban_refusal_exact", 500,
            (m,l,s) -> eq(l, "отказ", "отказываюсь"),
            (m,l,s) -> Result.banRefusal()));

        // ==================== 495: ПРЯМОЕ ПРИЗНАНИЕ (ключевое слово) ====================

        rules.add(new Rule("ban_confession_exact", 495,
            (m,l,s) -> eq(l, "я чит", "я читер", "признание"),
            (m,l,s) -> Result.banConfession()));

        // ==================== 490: ПРИЗНАНИЕ С ДОПОЛНИТЕЛЬНЫМИ СЛОВАМИ ====================
        // "я чит бань", "я читер бань", "я чит забань", и т.д.

        rules.add(new Rule("ban_confession_extended", 490,
            (m,l,s) -> {
                // Прямые фразы признания с любыми доп. словами
                if (has(l, "я чит ", "я читер ") && has(l, "бань", "забань", "кик", "давай бан")) return true;
                // "я чит" в начале с чем угодно после
        ...
        // ==================== 400: ОСКОРБЛЕНИЕ МОДЕРАТОРА (бан за неадекват) ====================
        // Бан ТОЛЬКО если мат направлен на человека/модератора.
        // Просто мат без направленности - НЕ банит.

        rules.add(new Rule("insult_moderator", 400,
            (m,l,s) -> {
                // Проверяем наличие оскорбительных слов
                boolean hasInsult = has(l,
                    ...
                    "пидор","пидр","педик","чмо","чмошник",
                    "твою мать","маму ебал","соси","пизд","шлюха","тварь",
                ...
                // Мат должен быть НАПРАВЛЕН на человека/модератора
                // Без направленности - не баним
                boolean directedAtPerson = has(l,
                    ...
                    "ты уебок","ты уёбок","ты уебан","ты мудак",
                    "ты козёл","ты козел","ты говнюк","ты скотина",
                    "ты крыса","ты шакал","ты мусор","ты ублюд",
                    "ты выродок","ты долбаеб","ты долбоеб","ты далбаеб",
                    "ты гандон","ты гондон","ты хуесос","ты педик",
                    "ты чмошник",
                    // Команды/посылание
                    "пошел нах","пошёл нах","иди нах",
                    "соси","катись","вали",
                    // Прямое указание на модератора
                    "модер сука","модер мразь","модер урод","модер дебил",
                    "модер даун","модер чмо","модер пидор",
                    "админ сука","админ мразь","админ урод","админ дебил",
                    // Косвенное направление через "тебя/тебе"
                    "тебя ебал","тебе пизд","тебя нах",
                    // Маму ебал - всегда оскорбление
                    "маму ебал","твою мать");
        ...
        // Игрок отказывается - сразу предлагаем оформить отказ, БЕЗ уговоров скачать программу
        ...
        // Игрок признаётся - сразу предлагаем оформить признание, БЕЗ уговоров скачать программу
        ...
        // ==================== 110: НЕ СКАЧИВАЕТСЯ -> ПЕРЕХОД НА RUSTDESK ====================
        ...
        // ==================== 15: МАТ БЕЗ НАПРАВЛЕННОСТИ (просто предупреждение) ====================
