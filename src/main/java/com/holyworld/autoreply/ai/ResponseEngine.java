...
public class ResponseEngine {
    ...
    public static class PlayerState {
        ...
        // Ключевое: после предложения RustDesk все ответы говорят RustDesk
        ...
        /** Возвращает название программы в зависимости от контекста */
        public String prog() { return useRustdesk ? "RustDesk" : "AnyDesk"; }
        /** Возвращает сайт программы */
    ...
    private void buildRules() {
        ...
        rules.add(new Rule("contextual_refusal", 350,
            (m,l,s) -> {
                if (!s.askedForProgram) return false;
                return eq(l, "нет","не","неа","нее","не буду","нет спасибо") ||
        ...
        rules.add(new Rule("leaving", 200,
            ...
            (m,l,s) -> {
                if (!s.warnedRefusal) { s.warnedRefusal = true;
                    return Result.reply("Выход = отказ от проверки (30 дней бана). Напиши \"Отказ\" если уверен. Проверка занимает 2 минуты."); }
                return Result.reply("Уход = отказ. Напиши \"Отказ\" или останься и пройди проверку.");
            }));
        ...
        rules.add(new Rule("cant_dl", 110,
            ...
            (m,l,s) -> {
                if (!s.useRustdesk) {
                    // Первый раз - переключаемся на RustDesk НАВСЕГДА для этого игрока
                    s.useRustdesk = true;
