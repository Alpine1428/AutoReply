...
public class MenuScreen extends Screen {
    ...
    private void btn(int cx, int sy, int g, int row, int w, int h, String label, boolean val, ButtonWidget.PressAction act) {
        addDrawableChild(ButtonWidget.builder(tog(label, val), act).dimensions(cx - w/2, sy + g*row, w, h).build());
    }
    ...
    public void render(DrawContext ctx, int mx, int my, float d) {
        ...
        ModConfig cfg = HolyWorldAutoReply.getConfig();
        ctx.drawCenteredTextWithShadow(textRenderer, "\u00a7b\u00a7lHolyWorld AutoReply v2.3", width/2, height/2-135, -1);
