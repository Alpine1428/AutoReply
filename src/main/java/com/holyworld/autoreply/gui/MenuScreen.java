...
public class MenuScreen extends Screen {
    ...
    protected void init() {
        ...
        btn(cx, sy, g, row++, w, h, "Главный переключатель", cfg.isEnabled(), b -> {
            cfg.toggleEnabled();
            if (!cfg.isEnabled()) { cfg.endCheck(); HolyWorldAutoReply.getChatHandler().getResponseEngine().clearAllStates(); }
            refresh();
        });
    ...
    private void btn(int cx, int sy, int g, int row, int w, int h, String l, boolean v, ButtonWidget.PressAction a) {
        addDrawableChild(ButtonWidget.builder(tog(l, v), a).dimensions(cx - w / 2, sy + g * row, w, h).build());
    }

    private Text tog(String l, boolean v) {
        return Text.literal((v ? "\u00a7aВКЛ" : "\u00a7cВЫКЛ") + " \u00a77| " + l);
    }
    ...
    public void render(DrawContext ctx, int mx, int my, float d) {
        ...
        String st;
        if (!cfg.isEnabled()) st = "\u00a7c\u2716 ОТКЛЮЧЕН";
        else if (cfg.isIdle()) st = "\u00a7e\u25CB Свободен";
        else if (cfg.isWaiting()) st = "\u00a76\u25CF Ожидание [CHECK]...";
        else st = "\u00a7a\u25CF Проверка: " + cfg.getCheckedPlayerName();
        ctx.drawCenteredTextWithShadow(textRenderer, st, width / 2, height / 2 - 122, -1);
        String spy = cfg.getLastSpyNick();
        ctx.drawCenteredTextWithShadow(textRenderer, "\u00a77Ник: " + (spy != null ? "\u00a7f" + spy : "\u00a78нет"), width / 2, height / 2 - 112, -1);
